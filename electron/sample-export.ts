...
const project: GaiaProject = {
  ...
  location: "İstanbul",
  client: "GAIA QA",
  description: "Beş analiz, iki jeoteknik birim, ortak kohezyon talebi ve kilitli model karar yoluyla kanonik çıktı parite doğrulaması.",
  coordinateSystem: "ITRF96 / TM30",
  verticalDatum: "TUDKA",
  selectedAnalysisIds: ["nonlinear-static", "construction-stage", "strength-reduction", "pile-soil", "consolidation"],
  groundUnits: [
  ...
  conditions: { groundwater: "static", drainageDecision: "both", constructionStages: true, dynamicLoading: false, interfacePresent: false, pilePresent: true },
  confirmedModelIds: Object.fromEntries(["nonlinear-static", "construction-stage", "strength-reduction", "pile-soil"].flatMap((analysis) => ["GU-SAND", "GU-CLAY"].map((unit) => [`${analysis}:${unit}`, ["mohr-coulomb"]]))),
