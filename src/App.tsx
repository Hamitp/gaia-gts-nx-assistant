        ...
        {contexts.map(({ analysisId, unit }) => {
          ...
          return <article className="model-context" key={contextKey}>
            ...
            <div className="model-cards">
              ...
              <button className={`defer-model ${deferred ? "selected" : ""}`} onClick={() => defer(contextKey)} aria-pressed={deferred}><CircleHelp size={18} /><span><strong>Modeli şimdi seçmeyeceğim</strong><small>GAIA bir model varsaymayacak; karar verebilmek için gerekli temel verileri isteyecek.</small></span>{deferred && <CheckCircle2 size={18} />}</button>
...
function ResultsStep({ result, project, onExport, onDetail }: { result: CanonicalResult; project: GaiaProject; onExport: () => void; onDetail: (req: ConsolidatedRequirement) => void }) {
  ...
  return (
    <div className="step-content result-step">
      ...
      {view === "tests" && <div className="result-table-wrap"><table className="result-table test-program-table"><thead><tr><th>Deney / standart</th><th>Sağlayacağı parametreler</th><th>Analizler ve birimler</th><th>İstenecek ham teslimler</th></tr></thead><tbody>{result.tests.map((item) => <tr key={item.id}><td data-label="Deney / standart"><strong>{item.method.nameTr}</strong><small>{item.method.nameEn}<br />{item.method.standardPrimary}</small></td><td data-label="Sağlayacağı parametreler"><div className="tag-list">{item.parameterIds.map((id) => <span key={id}>{parameters.find((p) => p.id === id)?.nameTr ?? id}</span>)}</div></td><td data-label="Analizler ve birimler"><p>{item.analysisIds.map((id) => analyses.find((a) => a.id === id)?.nameTr ?? id).join(" · ")}</p><div className="tag-list unit-tags">{item.groundUnitIds.map((id) => <span key={id}>{project.groundUnits.find((u) => u.id === id)?.name ?? id}</span>)}</div></td><td data-label="İstenecek ham teslimler"><ul className="raw-list">{item.method.rawDeliverables.map((raw) => <li key={raw}>{raw}</li>)}</ul></td></tr>)}</tbody></table></div>}
